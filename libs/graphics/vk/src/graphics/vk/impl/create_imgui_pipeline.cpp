...
[[nodiscard]] auto create_imgui_pipeline(VkDevice device,  //
    ...
    fmt::println("creating imgui pipeline");

    // TODO: create shaders
    auto const fs{cmrc::shaders::get_filesystem()};
    auto const vertexShaderSpirv{fs.open("imgui.vert.spv")};
    auto const fragmentShaderSpirv{fs.open("imgui.frag.spv")};
    ...
    if (vkCreateGraphicsPipelines(device, nullptr, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
        return std::unexpected{"failed to create imgui pipeline"};
    }
